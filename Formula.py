...
class Not(Formula):
    ...
    def evaluate(self,values):
        return not self.sub.evaluate(values) # code added
...
class Iff(Formula):
    ...
    def evaluate(self,values):
        return (self.first.evaluate(values) and self.second.evaluate(values)) \
               or (not self.first.evaluate(values) and not self.second.evaluate(values))
...
print("\n\n Here is the truthValues output: ", type(truthValues(myformula)))
print("\n example 2 ", )
